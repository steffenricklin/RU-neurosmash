...
class ConvVae(gluon.HybridBlock):
    ...
    """
    def __init__(self, batch_size=100, size=64, z_size=32, KL_tolerance_value= 0.5, beta=1, **kwargs):
        """
        size:
        """
        self.soft_zero = 1e-6
    ...
    #         encoder.add(Reshape((2*2*self.z_size,)))
        return encoder
    ...
    def getDecoder(self):
        ...
        decoder.add(nn.Dense(units=4*self.z_size))
    #         decoder.add(Expand_dims(axis=-1))
    #         decoder.add(Expand_dims(axis=-1))
        decoder.add(Reshape((4*self.z_size, 1, 1)))
    ...
    def hybrid_forward(self, F, x, *args, **kwargs):
        z = self.get_vae(F, self.encoder(x))

        y = self.decoder(z)
        self.output = y

        loss = self.compute_loss(F, x, y)
